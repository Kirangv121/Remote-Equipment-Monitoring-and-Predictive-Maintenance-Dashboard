...
function normalizeData(data) {
  ...
  const featureArrays = numericData.map((row) => features.map((feature) => row[feature]))
...
async function runPredictiveMaintenance(csvFilePath) {
  try {
    ...
    // Normalize data
    const { normalizedData, features, mins, maxs, originalData } = normalizeData(data)
    ...
    const trainErrors = tf.sub(trainXs, trainReconstructed).abs().mean(1).arraySync()
    ...
    console.log(`Detected ${anomalyCount} anomalies out of ${testData.length} test samples`)
